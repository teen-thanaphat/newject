...
app.get("/getdurablearticles/:durablearticles_Id", (req, res) => {
  const durablearticles_Id = req.params.durablearticles_Id;
  db.query("SELECT * FROM durablearticles INNER JOIN type_durablearticles ON durablearticles.type_durablearticles_Id = type_durablearticles.type_durablearticles_Id INNER JOIN company ON durablearticles.company_Id = company.company_Id WHERE durablearticles_Id = ?", durablearticles_Id, (err, result) => {
    if (err) {
      console.log(err);
    } else {
      res.send(result);
